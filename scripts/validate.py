...
def validate_thread_handoff_contract() -> None:
    ...
    required_text = (
        "one **START**, **RECOVER_START**, **MESSAGE**, **RECOVER_THREAD**, or **ROLLOVER_THREAD** operation",
        "Peer tasks are durable Codex threads, not child/subagents",
        "For a fresh repository START, call `list_projects` immediately before creation",
        ...
        "do not use `fork_thread` for START",
        "exact thread id",
        ...
        "## ROLLOVER_THREAD",
        "fork_thread` exactly once",
        "environment.type=same-directory",
        "A latest-turn 503 is neither necessary nor sufficient",
        "never consults project registration as workspace authority",
        "rollover_status: source_active",
        "rollover_status: uncertain",
        ...
        "report `native_start_status: unsupported` and leave START undispatched",
        "workspace_ownership: retained",
        "`repository_root` is the long-lived integration checkout",
        "`workspace_path` is the exact mutable checkout",
...
def validate_native_routing_contract() -> None:
    ...
    readme_required = (
        ...
        "saved-project registration is only native START addressing",
        "Codex-managed worktree remains authoritative",
        "same-directory rollover",
        "latest-turn 503 is neither necessary nor sufficient",
...
def validate_global_agents_template() -> None:
    ...
    required_text = (
        ...
        "A saved project only addresses a fresh native START",
        "task-bound Codex-managed checkout under `$CODEX_HOME/worktrees/`",
        "fork same-directory once and transfer mutable ownership",
...
def validate_git_lane_integration_contract() -> None:
    ...
    required_by_path = {
        ...
        WORKFLOW_PATHS["plan"]: (
            ...
            "saved codex project is only a fresh native start address",
            "`$codex_home/worktrees/` may be selected as `existing_worktree`",
            "git diff --cached --check",
            "review commit/range",
            "successor commit",
            "merge commit",
        ...
        WORKFLOW_PATHS["recover"]: (
            ...
            "task/worktree binding as workspace authority",
            "same-directory fork",
            "git diff --cached --check",
        ...
        WORKFLOW_PATHS["control"]: (
            ...
            "`repository_root` (the integration checkout)",
            "`workspace_path` (the exact mutable execution checkout)",
