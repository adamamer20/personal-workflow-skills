...
| --- | --- | --- |
| Current/history program documentation convention | Candidate `3a4125a` received P1 `DOC-POLICY-001`; bounded repair restores retained-contract reachability and capsule identity in both templates. Three causal tests, validator and full `make check` with 1,270 tests are green | Create successor commit, focused causal re-review, then reinstall matching plugin/tool and verify parity |
