...
| --- | --- | --- |
| Current/history program documentation convention | Candidate complete: stable per-program directory, normative current, non-normative history, optional evidence, PR-as-event and immutable dated snapshots without a documentation ledger; `plan-work` remains 7,981/8,000 bytes, validator and full `make check` with 1,269 tests are green | Coherent local commit, focused Sol Medium policy review, then reinstall matching plugin/tool and verify parity |
