...
Historical snapshot retained on 2026-09-07. This document is evidence, not
an execution queue or current lifecycle authority. Only the current plan may
select work or explicitly retain a named design as its active contract. Old
...
User request 2026-09-07: update harness instructions/operating practice and
reinstall the matching version. Supplied SprintAct audit and pasted analyses
are evidence/proposals, not independent execution authority. This selection
preserves the unfinished refresh-start-intent recovery below and its owned bytes.

### Outcome, risks and first proof

Make a new task choose the smallest experiment for material uncertainty, then
deliver an observable production vertical before general infrastructure. Keep
one outcome owner and controller authority, causal acceptance, necessary safety
and truthful separation of delivery, review, integration and installation.

The immediate risk is conflicting planning/review instructions, not an absent
runtime abstraction. Existing discovery/recovery skills cover the required
mechanics. First proof is a consistency walkthrough: uncertain work selects a
bounded experiment; a known small repair proceeds directly; two unsuccessful
causally identical repairs request diagnosis; an existing hard gate remains
binding. No new benchmark, runtime schema, public type or orchestration layer.

### Frozen scope and readiness

One instruction milestone, evidence-led-delivery-instructions, uses this exact
existing worktree and agent/python-sdk-controller branch. Base inspection HEAD
b3478d1b0ced5017b685f974ff82daf92e20f88b; preserve all pre-existing dirty
ledger/service/test/plan bytes and docs/reviews/conversational-tui. No fan-out,
new workspace or integration operation is needed. Planner owns this section and
root AGENTS synchronization; executor owns only the map below.

| Path | Action and responsibility |
| --- | --- |
| templates/AGENTS.md | modify: discovery before ready-milestone architecture; event-based diagnosis; preserve routing and authority |
| templates/AGENTS.workflow.md | modify: first feasible complete vertical, concrete foundation exception, compact current-plan view |
| plugins/personal-workflow-skills/skills/plan-work/SKILL.md | modify: resolve material unknowns before freezing speculative design; only ready milestone needs detailed map; reconcile review with current risk-triggered policy |
| plugins/personal-workflow-skills/skills/execute-milestone/SKILL.md | modify: after two attempts without new causal evidence/improvement, return diagnosis instead of a third identical attempt |
| plugins/personal-workflow-skills/skills/recover-milestone/SKILL.md | modify: distinguish oracle/outcome, implementation, scope and environment; choose one discriminating next check |
| README.md | modify: concise actual planning/controller/execution order and installation boundary |

No production module/class/protocol/registry/schema/entrypoint or dependency
changes. New durable artifact budget zero for execution. Reuse existing result
and evidence contracts. Current-plan guidance keeps live state, outcome, risks,
next milestone and next gate in front; history remains reference-preserving.
Do not relocate this active recovery history during an instruction-only edit.
No model changes, worker subagents, auto-retries, native fallback, global AGENTS
replacement, service refresh or installation in executor scope. Authoritative
runtime routing/permissions/idempotency remain unchanged. Existing accepted
capsules retain their required reviews. Visual acceptance retains independent
rendered judgment. Guarantee challenge cannot weaken an accepted safety gate.

DAG: instruction candidate -> focused consistency acceptance -> matching install.
The instruction work is independent of refresh implementation at the file level;
installation depends on accepted refresh-start-intent because the canonical
installer builds all current-checkout runtime bytes. This is a concrete shared
entrypoint/acceptance dependency. Do not publish the dirty recovery accidentally.
Installed preflight reports codex-flow 0.2.0, candidate schema 21, ledger schema
20 with migration required; current source recovery targets schema 22. A help
or schema compatibility result is not proof that dispatch/service is ready.
Do not open/migrate the live controller as an incidental instruction edit.
Execution readiness is pending safe existing-controller readiness; no alternative
transport is selected. Local instruction edits/review need no live provider.

### Acceptance and next gate

Check the four concrete scenarios above against every changed instruction,
including preservation of ready-milestone architecture, independent visual
judgment and controller-owned lifecycle. Run make test-workflow-assets and make
validate, then make check before packaging. Do not edit validator assertions to
silence a contradiction: return its exact rule for a bounded planning decision.
Use one focused Sol Medium architecture review for instruction consistency and
preserved ownership/guarantees; no duplicate review. Commit only owned files
after staged inspection and diff checks; never include pre-existing recovery.

Planner later synchronizes root/global guidance only within this accepted
instruction scope, preserving personal/local policy. Matching version bump and
canonical installation are a serial packaging step after candidate acceptance
and recovery promotion. Verify tool/plugin version and byte parity through the
existing installer; report global instructions separately. No live SDK sentinel
is authorized by the instruction milestone. Runtime redesign (free worker
subagents, deterministic automatic retries, new metrics store) is outside this
bounded update and is not inferred from the attached recommendations.

## Next execution — evidence-led-delivery-instructions

```python
ModelFacingCapsule(
    schema_version=1,
    objective="Make reusable harness instructions select risk discovery and the earliest complete observable vertical, with coherent proportional review and causal non-convergence diagnosis, preserving runtime authority and all existing guarantees.",
    decomposition=("Update the six mapped reusable instruction surfaces using existing discovery/recovery concepts, replacing conflicting prose rather than adding a second procedure.", "Walk through uncertain work, a small known repair, two causally identical failed attempts and an existing mandatory safety gate; validate and commit only owned instruction files."),
    acceptance_modes=(AcceptanceMode.ARCHITECTURE,),
    acceptance_criteria=("Material unknowns are resolved by existing evidence or bounded discovery before speculative architecture is frozen; the next ready milestone retains a decision-ready map and first feasible production vertical.", "Review policy is coherent and risk-triggered without weakening accepted capsule gates or independent visual judgment; lifecycle, ownership, permissions, routing and leaf boundaries remain unchanged.", "Two attempts without new causal evidence or observable improvement return bounded diagnosis; no automatic retry, model change or third indistinguishable repair is introduced.", "Four scenario walkthroughs, workflow-assets partition and validation pass; owned instruction-only commit is delivered with focused independent acceptance and installation pending."),
    mutable_surfaces=("templates/AGENTS.md", "templates/AGENTS.workflow.md", "plugins/personal-workflow-skills/skills/plan-work/SKILL.md", "plugins/personal-workflow-skills/skills/execute-milestone/SKILL.md", "plugins/personal-workflow-skills/skills/recover-milestone/SKILL.md", "README.md"),
    protected_surfaces=("src", "tests", "scripts", "workflow.toml", "AGENTS.md", "docs/reviews/peer-thread-workflow.md", "docs/reviews/conversational-tui", ".agents", "plugins/personal-workflow-skills/.codex-plugin"),
    authorities=(ModelAuthority(AcceptanceMode.ARCHITECTURE, RoleId("architecture-reviewer")),),
    prompt="Use execute-milestone for the Instruction update evidence-led delivery section of this canonical plan. Own only its six instruction paths. Preserve all pre-existing dirty recovery and plan bytes. Use existing discovery and diagnosis concepts, no runtime changes or relaxed safety gates. Replace conflicting prose and keep model routing unchanged. Validate the four named scenarios, make test-workflow-assets and make validate; return exact validator conflicts instead of changing protected tests/scripts. Self-review and commit only owned files after staged checks. Do not install, dispatch, use subagents, change global state, or relocate the current plan. Return candidate, evidence and focused architecture acceptance pending.",
...
REPLAN_NOTICE at exact candidate b3478d1b0ced5017b685f974ff82daf92e20f88b,
agent/python-sdk-controller, existing program worktree. Sole focused Sol Medium
review rejected ROTATION-START-REENTRY-DOUBLE-START (P1). Prior profile-rotation
re-entry test explicitly expects a second start after an uncertain/nonzero first
start; that expectation must change. Preserve all implementation and history in
planning; only this canonical plan changes, untracked conversational-tui protected.

### Decision and observable limit

Existing observations cannot distinguish two histories: (a) stop/migration/profile
staging completed but no start was requested, and (b) start was submitted but its
reply failed before any new authority/socket/live process became visible. Both
can show the same inactive/MainPID=0 unit, target bytes, old fenced authority and
absent socket. Repeated observations or time passage do not resolve a submitted
manager job. An invocation-local possible-replacement flag loses this distinction.
Therefore add a bounded durable start-intent record to the EXISTING SQLite ledger,
not a sidecar/control service or manager-job guessing mechanism.

This is one coherent recovery milestone refresh-start-intent. Do not split a
schema-only precursor that would leave the start boundary unguarded. Keep one
Astra Low mutable recovery owner and one reviewed successor commit. Planning here
freezes the required public/persisted extension; no user decision is needed because
it preserves the explicit no-second-start guarantee.

### Frozen persistence and API

Add one table service_refresh_starts in ledger.py, SQLite schema 21 -> 22, with
existing canonical schema validation/fingerprint/migration machinery. Columns:
repository_root, state_root, predecessor_epoch, predecessor_pid,
predecessor_birth_identity, predecessor_authority_sha256, unit_name,
unit_sha256, launcher_sha256, interpreter_sha256, target_profile_sha256 (nullable),
target_compatibility_sha256 (nullable), intended_schema_version, state,
created_at, resolved_at (nullable), replacement_authority_json (nullable).
Primary key (repository_root, state_root, predecessor_epoch); state is exactly
pending, healthy, or stopped_replacement. Existing authority/JSON codecs and
bounded SHA/identity validation apply; do not add public record classes or tokens.
No raw credential values, provider details or command output are stored.
One row per actual refresh start intent is durable history; no row deletion or
pending-to-absent reset. No legacy rows are backfilled as attempted or successful.

Existing Ledger gains arm_service_refresh_start and resolve_service_refresh_start,
plus service_refresh_start read method, using existing JsonObject at this ledger
boundary. Arm validates exact current fenced authority, empty active children/
claims, target identity and no existing predecessor intent, in one transaction.
Identical pending replay returns its pending disposition and DOES NOT grant a new
start; conflicting identity fails. Only the caller that newly inserts the row
receives permission to issue the one manager start. A process crash immediately
after insertion and before command is conservatively uncertain on restart.

Resolve records only an exact observed replacement with greater epoch and distinct
PID/birth satisfying existing identity checks: healthy requires existing complete
live/unit/executable/fence-clear proof; stopped_replacement requires captured exact
acquired replacement fenced then proven dead, unit inactive/MainPID=0 and sockets
absent with retained target bytes. Compare expected pending row and authority
under existing ledger transaction. Retain original pending intent identity forever;
resolution appends facts to its row without rewriting predecessor. A later refresh
for a proven replacement uses its newer epoch, not another row for the old epoch.
No explicit reset, abandon or force-start public command is added.

Migration schema 22 is consumed by this safety repair; the later unimplemented
program-user-approval migration moves from planned 21->22 to 22->23, preserving
its dependency on this integrated tip. No approval behavior is implemented here.
New empty table preserves every old authority/dispatch/history row. Because current
21 becomes a predecessor, extend the EXISTING supported predecessor table/fence/
inspection and service migration selectors to include 21->harness_authority; do
not introduce another v18-only regression. v18 remains supervisor, 19/20/21 current
harness topology. Expected-authority migration guards must cover the full chain;
no migration while an old process/child/claim is active. All current-schema checks
use CURRENT_SCHEMA_VERSION rather than stale numeric 21 assumptions.

### Bounded fence API correction — pending replacement ownership

Decision: accept the worker proposal. The protected canonical installer invokes
arm_harness_refresh_fence before replacing the shared tool and before service
reconciliation. Service may also race a new pending replacement after an earlier
read. A service-only check cannot protect either race; the default ledger fence
must refuse a pending start atomically. No installer source change is authorized.

Freeze existing method signature as:
`arm_harness_refresh_fence(self, *, expected_start: Mapping[str, object] | None = None, expected_authority: Mapping[str, object] | None = None) -> JsonObject | None`.
Both optional values must be present together or both absent; half-pairs fail
ValueError before mutation. Keep existing return shape and existing callers.
Use existing closed start-row/authority validators, not a new public type or
service SQL. Existing constructor/migration/fence semantics otherwise remain.

Default (both absent): within the SAME existing write transaction used for child/
claim checks, authority read and shutdown-bit write, inspect ALL pending
service_refresh_starts rows in this ledger, not merely rows matching the caller's
root or current epoch. Any pending row raises HarnessRefreshBlocked before fence
mutation, even if requested_shutdown already equals 1 or authority is absent.
Validate retained rows; malformed/ambiguous multiplicity fails closed. No pending
row preserves accepted normal fencing behavior, including child/claim guards.
A stale earlier service observation never substitutes for this transaction check.
The guard must execute before the shared tool can be replaced by an unchanged
installer. No pending-intent resolution, reset or lifecycle effect occurs here.

Paired override: only service's existing acquired-unhealthy replacement recovery
may supply the exact retained pending row and freshly inspected acquired authority.
Within that same transaction require exactly one pending row, byte/canonical-field
equality with expected_start, matching repository/state root, and exact current
authority equality with expected_authority using the existing row contract.
Require current epoch strictly greater than pending predecessor epoch, distinct
PID/birth pair, valid current authority/fence bit, matching intended schema/version
and interpreter digest, plus all existing child/claim checks. Unit/launcher/profile
identity and unhealthy classification remain service-owned proofs immediately
before calling; ledger is not a second manager or health checker. Wrong-root,
missing/resolved/conflicting pending row, stale/malformed authority, unchanged
predecessor identity, or competing pending rows rejects without writing the bit.
Only after checks set requested_shutdown=1 and return the exact fenced authority.
If already 1, validate identically and return without a redundant update. Do not
weaken exact matching merely to allow a lease-renewal race: service may return a
truthful stale failure and reconcile afresh, never guess or overwrite new owner.

Override does not resolve pending state, authorize another manager start or claim
that replacement is dead. Existing exact authenticated/manager stop, birth death,
socket absence, target-unit retention and resolve_service_refresh_start CAS must
follow before stopped_replacement resolution. Healthy pending replacement follows
the existing direct service health reconciliation and never calls either default
fence or paired unhealthy fence. Default installer failure remains explicit:
controller must reconcile through the existing service boundary before retrying
installer; no installer bypass, implicit healthy-owner stop or automatic retry.
A pending replacement that becomes healthy or changes identity between service
classification and the paired fence must fail existing fresh identity/health
checks; tests freeze that observation seam, not an adversarial global lock model.

One consistent transaction serializes arm_service_refresh_start/default fence and
paired fence against acquire/resolve updates. If a new pending row or newer owner
wins before default fencing, fencing rejects; if paired CAS loses, it rejects.
Do not place a pending check only outside BEGIN or call an accessor whose result
is then used as an unchecked write permission. No schema/table additions beyond
the already frozen intent table; all six owned paths and single focused Sol Medium
ROTATION-START-REENTRY-DOUBLE-START review remain unchanged.

Required focused tests within owned ledger/service/installer tests:
- pending start later produces exact healthy replacement; repeated canonical
  installer default fence refuses before shared-tool replacement, shutdown, unit
  staging or start, preserving the healthy row/fence and pending intent;
- direct public refresh reconciles that healthy pending replacement and resolves
  it without fence/start, including lost-reply idempotency;
- interleave normal refresh's initial no-pending observation with a committed
  pending intent/new acquired owner before its default fence; reject with exact
  authority/tool/intent unchanged, using real disposable ledger transactions;
- exact acquired-unhealthy pending replacement succeeds only with the paired CAS,
  then existing stop/resolution proof; missing half-pair, wrong/stale/resolved
  row, wrong root/epoch/PID/birth/digest, active claim/child and ambiguous pending
  rows reject without mutation; already-fenced exact pair remains idempotent;
- no-pending ordinary fence, legacy/predecessor migration and accepted lifecycle
  behavior retain parity. Do not change the four current-schema assertions or one
  DROP tuple beyond their already authorized mechanical updates.

### Exact ordering and re-entry cases

Before any fencing/staging/start on a repeated invocation, inspect retained start
intent for the exact state-root/predecessor and current authority. Never allow
ordinary refresh logic to overwrite a possibly healthy replacement before
reconciling the pending start. Reconcile as follows:

| Observation | Allowed behavior |
| --- | --- |
| No intent, complete accepted stopped proof | Proceed through existing migration/profile authorization/staging/reload/import and final stopped proof; atomically arm intent immediately before start, then issue exactly once. |
| Pending intent, exact healthy greater-epoch replacement | Revalidate complete health/target identity, resolve healthy, return reconciled refresh success without stop/start. |
| Pending intent, exact acquired unhealthy greater-epoch replacement | Reuse accepted exact fence/stop/death/socket/retained-unit proof, resolve stopped_replacement; return failure with that newer owner safely stopped. A later explicit invocation may start only under its new predecessor epoch. |
| Pending intent, unchanged dead fenced predecessor, no socket, inactive/MainPID=0 | Still ambiguous: no second start. Return ServiceRefreshFailed identifying unresolved prior refresh start, preserve current state. No timeout-based promotion to absence. |
| Missing/malformed/drifted/unknown authority, unit or executable identity while pending | Fail closed, preserve intent/schema/unit and no start. |
| No intent and no valid stopped proof | Existing fail/defer semantics; absence of a row never replaces positive lifecycle proof. |

Positive no-replacement classification is available only when this implementation
has not created/submitted a start intent and all existing stopped proofs pass.
For a persisted pending intent, present manager/process absence is insufficient;
there is intentionally no automatic no-replacement inference. A lost/uncertain
start with no later acquired authority remains blocked until new external evidence
allows a separately planned recovery; do not invent that action now. This honest
conservative crash window is preferable to a second unsafe start. Freshly created
intent is permission only within its creating invocation, never replay permission.

After intent commit, nonzero/raised/start timeout retains pending forever until
positive replacement reconciliation; never delete it in exception cleanup. Even
health success followed by lost ledger resolution is safe: restart sees pending,
proves exact healthy replacement and resolves without another start. Fault after
resolution returns existing result idempotently. Preserve profile rotation,
v18 compensation's existing proof requirements, current forward-only schema/unit
behavior and no manual unit mutation. v18 unit restoration never clears a pending
start or grants a second start; unresolved-start exclusion remains stronger than
any legacy compensation path. No downgrade, manual fence clear or workload replay.

Pre-existing pre-upgrade uncertain starts cannot be reconstructed from a missing
new row. Before controller cutover, it must reconcile the exact live installation
history and ensure no unresolved submitted start precedes the upgrade, or return
that uncertainty without invoking start. No automatic backfill or assertion that
absence of a newly introduced row proves historical absence. This prerequisite
belongs to later installation reconciliation, not provider-free implementation.

### Exact map, proof and review

| Path | Action/responsibility |
| --- | --- |
| src/codex_flow/ledger.py | modify: one table/migration/version identity, bounded arm/read/resolve API, predecessor schema 21 support; sole durable state owner |
| src/codex_flow/service.py | modify: intent-before-start and re-entry reconciliation ordering using existing accepted identity/liveness/profile proofs; include supported current-topology v21 predecessor |
| tests/test_ledger_integrity.py | modify: schema chain/old-state preservation, unique concurrent intent, replay/resolve CAS and predecessor-21 fencing |
| tests/test_service_lifecycle.py | modify: correct violating second-start expectation and public crash/re-entry matrix; accepted v18/profile/manager/executable/claim/socket parity |
| tests/test_plugin_installation.py | modify: disposable canonical repeated-install flow never submits second start after unresolved first; current-21 migration selection |
| tests/test_controller_execution.py | modify ONLY four current-schema identity assertions at baseline lines 4228, 4251, 4276 and 4403 plus the single historical-fixture DROP tuple addition specified below; use the exact schema-22 identity string declared by this accepted migration, matching existing literal-assertion style |

All other paths protected, including installer production code, contracts/domain,
SDK/IPC transport, runtime evidence, approval capability, AGENTS/routing, dependencies,
test partitions, canonical plan (executor cannot stage), TUI artifacts and every
live/runtime/global/auth/downstream surface. No new files/modules/classes/commands,
no second lifecycle authority. Service depends on existing Ledger; ledger never
imports service. If version-identity fixtures outside these six paths require
changes, return exact failing fixture evidence for a bounded path-only update
before editing it; never disable migration checks to meet the file limit.


Bounded ownership correction: tests/test_controller_execution.py is authorized
for the four literal current-schema identity assertions identified at
baseline lines 4228, 4251, 4276 and 4403 (each follows Ledger(..., migrate=True)).
Replace the stale codex_flow_program_outcomes_v21 expectation with the exact
schema-22 identity introduced by the already frozen migration; do not derive an
identity by guessing a version suffix or weaken the check to nonempty/inequality.
Preserve historical v21 fixture identities, behavior, imports and every
other assertion in that file; the sole additional setup exception is below. This is mechanical schema-fixture alignment only;
architecture, migration behavior and the single focused Sol Medium re-review lens
are unchanged. Line shifts do not expand ownership beyond these four assertions and the one
DROP tuple addition below.

Second bounded fixture correction: in
`test_migrated_v7_predecessor_without_terminal_snapshot_requires_explicit_reconciliation`,
add exactly `"service_refresh_starts"` to the existing DROP-table tuple near
baseline line 4442 in tests/test_controller_execution.py. This fixture reconstructs
v7 from a current ledger; leaving the new table causes strict sqlite_master
validation to reject residue at Ledger(database, migrate=True). Remove that residue
through the existing fixture tuple only. Preserve its historical semantics and
strict production _validate_shape; no other test edit is authorized by this
exception. The four current-schema assertions remain authorized. Worker reports
22 new intent/replay/crash/concurrency/resolution/migration causal checks passing;
that supplied progress does not replace the remaining gates or focused review.
Architecture, six-path ownership and single Sol Medium review routing are unchanged.

First falsification checkpoint: through public refresh create pending intent,
make systemctl start return nonzero or raise before authority changes, close/reopen
ledger and invoke refresh again with inactive/MainPID=0/no sockets/old fenced
predecessor. Total start calls across BOTH invocations must remain one. Prove
crash after intent commit but before manager call yields zero subsequent starts.
Concurrent calls racing to arm result in at most one permitted start. Inject a
late exact healthy replacement and show success resolution without another start.
Inject acquired unhealthy replacement and prove controlled stop/resolution, then
one later explicit start with NEW predecessor epoch only. Different launcher/unit/
profile/state root/authority identity rejects conflicting replay. No same-epoch
reset or elapsed-time bypass.

Test failures at intent commit, immediately after commit, manager request/reply,
health observation, resolution commit and lost resolution reply. Preserve old
historical rows across 18/19/20/21->22 upgrade, rollback-on-migration failure and
schema-22 reopen. Assert public repeated canonical installer/service behavior with
temporary ledgers/fake manager/process seams, no real installer/SDK/service run.
Full make check plus affected service/ledger/installer partitions required because
persisted/shared boundaries change. Stage exact owned paths after self-review,
git diff --cached --check and one coherent successor commit; never amend candidate.

Risk-triggered acceptance: ONE focused Sol Medium re-review of
ROTATION-START-REENTRY-DOUBLE-START only, including intent/replay/migration mechanism
needed for its closure. No duplicate review. Existing accepted closures are reused;
new material ambiguity returns to controller, not automatic escalation. Promotion
requires this P1 closed/no open promotion blockers. Parent alone owns future
installation reconciliation; no retry by planner or executor.

Readiness refresh-start-intent READY after controller accepts frozen durable
extension. Next recovery owner Astra Low, same workspace/candidate lineage;
planning Astra Medium. CONTINUE_WITH_REPLAN, no user decision required.
...
## Prior recovery — explicit refresh profile rotation

REPLAN_NOTICE. Source inspected at policy commit
2ca7b891a0d5eb9b37abbe0438fd024a7ffa7bf4 after accepted runtime
cbf4700c9479b320860db0817f6c8aa56d6b4d3a. Only canonical plan modified;
implementation, live state and untracked conversational-tui remain preserved.

Parent-supplied installation evidence, not refreshed from live state by this leaf:
canonical retry completed preflight, harness-refresh-fenced, controller-installed,
then failed "installed service unit profile identity drifted". Exact unit
codex-flow-937220b34ee45f7f.service is inactive/dead, MainPID=0 with expected
repository/ExecStart/current harness topology. No plugin/cache update occurred.
Preserve fence and historical state. Installed unit profile is
c3c95ef9bba41a1ccb07e24c6a8af63f10ae4a68c4d3f2930f0cd6b2b43e6494;
current projected profile is
eea5397276b6df06672c0184d7d67a669968e7f7c4fcccfb2210f58fa2024258,
compatibility 2d03425118965aecd69227c23fa0eb525d62fe6882141e3d532b951176aea267.
These are retained observations, not hardcoded production constants.

Diagnosis near_completion: legitimate rotation is admitted at preflight but
same-topology pre-migration validation prematurely requires target profile; the
migration path also skips subsequent current unit installation. Existing
Ledger.authorize_controller_profile_refresh is already the explicit authorization
boundary and limits changes to pre-identity decisions. Its contract/implementation
remain protected. No new authority, schema, public API or recovery record needed.

### Frozen sequence and failure states

One bounded repair refresh-profile-rotation, one Astra Low recovery owner in the
same worktree /home/adam/personal-workflow-skills.worktrees/python-sdk-controller,
branch agent/python-sdk-controller. Preserve accepted v18 compensation, exact
manager show/MainPID, executable identity, claims/instant ordering, sockets,
expected-authority migration and no uncertain-start rollback.

1. Preflight admits only the existing strict allow_profile_identity_update=True
   validation: exact expected unit except one well-formed profile line in its
   required location. Capture returned installed unit digest, raw bytes/mode and
   old profile line from that validated snapshot using existing safe reader.
   This identifies the predecessor artifact; it does not authorize changing it
   or authorize active work. Retain current projected profile/compatibility as
   separate target facts. Reject missing/duplicate/malformed line, any other
   content/ExecStart/environment-key drift, or changing unit bytes during capture.
2. Fence/check children and claims, authenticated shutdown if necessary, exact
   predecessor death/current unit inactivity/MainPID=0/socket absence and launcher
   identity proof remain mandatory. At same-topology pre-migration boundary use
   profile-rotation-tolerant structural validation PLUS equality to the captured
   OLD unit digest/bytes/mode; do not replace strict target comparison with an
   unbounded any-profile allowance. Re-read it before migrating. The target unit
   has not been installed yet. Expected predecessor authority remains exact.
3. Migrate only after those stopped-owner proofs via existing
   expected_refresh_authority transaction. Preserve old unit bytes through this
   migration. On current-schema stopped re-entry migration is skipped but capture
   and validate whichever exact well-formed old/current-profile unit is retained;
   reconstruct all stopped proofs from durable facts, not invocation history.
4. With canonical ledger available and old owner stopped, call existing
   authorize_controller_profile_refresh with the exact current profile and
   compatibility. Preserve its eligibility guards/idempotency; never rotate
   active/identified work, permissions, capsule/plugin facts or historical rows.
   Require the pair supplied as the existing explicit refresh contract requires;
   do not invent a missing identity, scrape config or create an alternate writer.
   Reuse its return/result semantics: skipped identified decisions stay unchanged,
   not secretly rebaselined to make refresh green.
5. Immediately after successful authorization, revalidate stopped authority,
   children/claims and captured unit/launcher (accounting only for legitimate
   profile authorization's decision changes), then install current-profile unit
   through existing install_unit for same-topology migration as well as ordinary
   current refresh. This is the first safe point for its replacement in this path.
   Strict _validate_installed_unit with allow_profile_identity_update=False must
   verify the target profile and complete target bytes after staging. Capture
   target unit identity for remaining proofs; this explicit staging is the only
   authorized old-to-new unit identity transition. Do not retain the old digest
   as target or silently accept any profile after this point.
6. Reload/import, repeat strict target-unit/stopped-owner/socket/manager/executable
   proof, set existing monotonic possible-replacement guard and start once. Exact
   healthy newer authority/epoch/fence-clear remains success criterion. Preserve
   failure credential cleanup and content-free reporting.

| Failure stage | Required durable/re-entry behavior |
| --- | --- |
| Before migration | Old schema, captured old unit and fence retained; no target authorization/start. Fresh explicit invocation repeats all proofs. |
| Migration fails | Existing transaction determines old vs committed current schema; inspect honestly, retain old unit/fence, no downgrade or current-to-v18 compensation. |
| Migration committed, authorization fails | Current schema, old unit, fence retained; no unit staging/start. Re-entry reuses canonical ledger and exact old unit, calls same authorization boundary. |
| Authorization committed, staging fails before write | Current schema and authorized eligible profile facts remain; old unit/fence retained; no rollback of decision history. Same target authorization is idempotent on retry. |
| Staging raises after target bytes published | Strictly distinguish exact old unit, exact target unit or invalid partial/drifted unit using safe reader. Exact old/target may enter a fresh fully checked forward retry; malformed/partial fails closed, never guessed/repaired blindly. |
| Reload/import fails after staging | Retain strict current unit/current schema/authorized profile/fence; no start and no rollback. Fresh invocation checks this target unit and authority before continuing. |
| Start uncertain/nonzero or malformed health | No second start, downgrade, unit restore or fence clear. Preserve accepted uncertain-owner reconciliation. |
| Exact acquired unhealthy current owner | Existing fence/stop/socket/retained-current-unit proof applies; no legacy compensation. Later explicit retry only after fresh safe stopped proof. |

A fresh projected target may legitimately change between explicit invocations;
validate old retained unit and the newly supplied target through the same explicit
boundary, never relabel an active/identified route. No cached prior approval or
elapsed time authorizes this. Same-topology forward-only behavior stays distinct
from unchanged v18 topology-specific unit staging/compensation. Do not move v18
staging merely to share code; preserve its accepted order and gates.

### Exact map, tests and acceptance

| Path | Action/responsibility |
| --- | --- |
| src/codex_flow/service.py | modify: predecessor-vs-target unit/profile identity and same-topology sequencing inside refresh_with_credential; existing ledger/reader/install helpers only |
| tests/test_service_lifecycle.py | modify: public old-profile v20 migration/current re-entry and fault injection around authorization/staging/start |
| tests/test_plugin_installation.py | modify: disposable canonical installer/service flow with rotated profile; cache/plugin withheld on refresh failure |

All other paths protected, especially ledger.py and authorize_controller_profile_refresh,
manager/executable producer contracts, schemas/domain, installer production code,
AGENTS/routing/global/auth, canonical plan (executor cannot stage), untracked TUI,
live service/ledger/tool/plugin/cache and downstream/app-load state. No new module,
class/public API/schema/table/command/transport/durable artifact. Private helper
changes stay inside service.py, service -> existing ledger/OS/manager boundaries.
If existing authorization cannot satisfy an observed valid case, return exact
counterexample for bounded planning before changing ledger guards.

First discriminating test: real disposable schema v20, old-profile current unit,
new installed canonical launcher, fenced dead predecessor and no sockets, manager
inactive/MainPID=0. Call public refresh_with_credential with new profile/compatibility.
Assert exact order: old snapshot validated and unchanged through migration ->
existing authorization -> new unit publication -> strict target validation ->
reload/import -> final stopped proof -> one start/new healthy authority. Assert
eligible pre-identity decisions rotate and identified/active/history/permission/
plugin/capsule facts do not. Use actual ledger API/temporary state, no service SQL.
Also test live predecessor shutdown variant and current-schema fenced/dead
old-profile re-entry, with zero automatic workload/provider calls.

Fault matrix covers before migration, transaction rollback/post-commit opening,
authorization before/after its commit, staging before/after publication, reload,
import, uncertain start and unhealthy acquired replacement. At every fault assert
actual unit bytes/profile, schema, fence, exact unchanged identified rows and number
of starts. Re-enter with exact old and target units independently; idempotent
profile authorization must not hide stale owner/claims/sockets/launcher/unit drift.
Add malformed/unexpected old profile/unit mutation rejection; final strict target
validation must still reject drift. Retain all accepted v18/manager/executable/
claim/socket/current-recovery tests; no unrelated rewrite of their proof.

Run focused service/installer tests, affected service/ledger/installer partitions
and full make check as the explicit shared-service closure gate. Reuse accepted
unchanged evidence. Self-review full/staged diff, git diff --cached --check and one
coherent three-path successor commit without amend/discard. Concrete residual risk
is the authorization-before-unit-publication crash window and preservation of
identified decisions. Therefore require ONE focused Sol Medium review of that
sequencing/failure boundary and old-vs-target identity, with no promotion blockers.
No automatic duplicate objective review or broad re-review; add authority only for
new material ambiguity via controller. Source delivery/tests do not mean installed.

Readiness READY after parent accepts plan. Route Astra Low bounded implementation
recovery; Astra Medium planning here. Parent alone dispatches focused review and
considers installer retry after exact repair acceptance. No retry/runtime action
is authorized to this leaf or executor. Approval capability/downstream activation/
TUI remain deferred. CONTINUE, no user decision required.

## Superseded capsule — refresh-profile-rotation

```python
ModelFacingCapsule(
    schema_version=1,
    objective="Repair refresh-profile-rotation: preserve exact old-profile current unit through predecessor stop and schema migration, authorize current profile through the existing ledger boundary, then stage and strictly prove the new-profile unit before one replacement start.",
    decomposition=(
        "Capture exact validated predecessor unit identity separately from target profile and require unchanged old bytes through same-topology migration.",
        "Reuse existing explicit profile authorization then stage target unit for migrated and current-schema paths; preserve v18 order and all accepted stopped-owner/start safety.",
        "Prove old/current-unit forward-only re-entry and all migration/authorization/staging/start fault windows in disposable public-entrypoint tests, run required gates and commit only owned paths.",
    ),
    acceptance_modes=(AcceptanceMode.ARCHITECTURE,),
    acceptance_criteria=(
        "Old-profile schema-v20 fenced/dead current unit plus new launcher completes ordered migration/authorization/target staging and strict target proof without profile-drift false rejection.",
        "Only existing eligible pre-identity decisions rotate; active/identified/history/permission/plugin/capsule facts remain unchanged and no service raw SQL or new profile writer exists.",
        "Failures before/after migration, authorization, staging and start preserve truthful unit/schema/fence state; exact old/current-unit re-entry is safe and idempotent, malformed/uncertain state never starts twice or downgrades.",
        "Accepted v18/manager/executable/claims/socket/current recovery guarantees remain; required gates pass, one exact successor is delivered with one focused Sol Medium sequencing review pending and no installer retry.",
    ),
    mutable_surfaces=("src/codex_flow/service.py", "tests/test_service_lifecycle.py", "tests/test_plugin_installation.py"),
    protected_surfaces=("docs/reviews/peer-thread-workflow.md", "docs/reviews/conversational-tui", "src/codex_flow/ledger.py", "src/codex_flow/harness.py", "src/codex_flow/domain.py", "src/codex_flow/contracts.py", "scripts/install_personal_workflow_skills.py", "schemas", "config/test-partitions.toml", "workflow.toml", "AGENTS.md"),
    authorities=(ModelAuthority(AcceptanceMode.ARCHITECTURE, RoleId("architecture-reviewer")),),
    prompt="Use recover-milestone as Astra Low bounded recovery for refresh-profile-rotation on the current accepted cbf4700 runtime plus 2ca7b89 policy baseline in this same worktree. Implement only Current recovery sequence and three owned paths. Distinguish captured old unit/profile from target; keep old bytes through stop/migration, call existing authorize_controller_profile_refresh, stage target then strict validate before reload/import/final proof/start. Preserve v18 compensation and all accepted manager/executable/claims/socket/uncertain-start guarantees. Prove first disposable old-profile v20 case and fault/re-entry matrix; no ledger edits or service SQL. Run focused/affected/full make check, self-review/staged diff/git diff --cached --check and one coherent successor commit without amend. Preserve dirty plan/TUI and every unowned byte. No live installer/service/unit/ledger/global/plugin/cache/auth/provider/SDK/downstream mutation, subagents/peers/reviews/workflow-control or retry. Return exact candidate/evidence with one focused Sol Medium sequencing review pending; parent owns lifecycle. Missing authorization capability requires exact counterexample before scope expansion.",
    recovery_policy="completion_biased",
)
```

## Accepted recovery — manager state and claim history

REPLAN_NOTICE for internal CONTINUE_WITH_REPLAN at exact candidate
4572917e23e40e4f6df0b50ad40c073689f294b6 in the existing
/home/adam/personal-workflow-skills.worktrees/python-sdk-controller worktree,
branch agent/python-sdk-controller. No implementation bytes changed in this
planning turn. Preserve accepted executable/socket/current-recovery/v18 closures,
prior commits, dirty canonical plan and untracked conversational-tui artifacts.
Three remaining promotion blockers only: REFRESH-CLAIM-HISTORY-REJECTED (P1),
REFRESH-MANAGER-LIVE-PID-UNPROVEN (P1), REFRESH-CLAIM-TIME-ORDERING (P2,
promotion_blocking=true). No installation retry or live state mutation is granted.

### Frozen smallest private manager extension

Inspection confirms _CommandResult guarantees only returncode; _run_manager drops
captured stdout, and _unit_is_active accepts returncode 3 as inactive without
MainPID evidence. Add stdout: str to the EXISTING private _CommandResult Protocol.
Keep structural fake-runner injection, never assume subprocess.CompletedProcess.
Add one private _manager_result(arguments, runner) helper that calls the existing
runner once with check=False, capture_output=True, text=True, validates a non-bool
integer returncode and string stdout, and returns that protocol. Existing
_run_manager becomes the integer projection of that validated result, preserving
its call-site interface. No new Protocol, wrapper record, public API, runner or
transport. Update owned fake result fixtures to expose stdout (empty for commands
that do not return state). Missing/wrong stdout or returncode fails visibly;
getattr/default/coercion is forbidden. Do not emit raw captured output in errors.

Add private _assert_unit_stopped(unit, runner) for proof boundaries. Issue exactly
one command per observation:
`systemctl --user show <unit.unit_name> --property=ActiveState --property=MainPID --no-pager`.
The unit name is the exact already validated ServiceUnit binding, never a wildcard,
PID-derived guess or another service. No shell, no second separate property call
and no fallback to is-active. Require returncode 0 and a bounded ASCII response
(maximum 256 bytes) with exactly two nonempty lines, either order, one of each key:
`ActiveState=<value>` and `MainPID=<value>`. Permit one optional terminal LF only;
reject extra blank lines, CR, leading/trailing whitespace, missing/duplicate/
unexpected properties and malformed separators. Parse MainPID using ASCII decimal
`0|[1-9][0-9]*`, at most ten digits, nonnegative integer (no signs/floats/coercion).
The sole successful stopped observation is ActiveState=inactive AND MainPID=0.
Active/failed/transitional/reloading/unknown states, nonzero MainPID, nonzero exit
or malformed output cannot prove stop. During existing bounded waiting phases a
well-formed non-stopped observation may continue waiting; at a required final proof
it rejects. Malformed response fails immediately; no endless polling/retry.

Use this joint observation at EVERY stopped-owner proof boundary, including
_prove_stopped_current_owner, legacy interrupted preflight, positive absence proofs,
post-stop unhealthy-replacement recovery and the final proof before migration or
start. _unit_is_active may remain for ordinary positive health/wait decisions,
but no false result from it can authorize migration/start/restoration/retry. A
wait-loop exit must be followed by the joint manager proof before its effect.
Keep birth-bound OS liveness, exact authority/fence/children/claim revalidation,
unit bytes/socket absence and invocation-independent re-entry checks unchanged.
MainPID=0 complements these checks; it is not a replacement for them. No claim of
atomicity against arbitrary post-observation OS changes; preserve existing final
revalidation/CAS lifecycle boundaries.

### Two unchanged ledger repair decisions

REFRESH-CLAIM-HISTORY-REJECTED: _live_controller_claim_count_in_transaction must
read decision state alongside retained claim fields and apply existing lifecycle
state-specific shape rules. Legitimate terminal/acknowledged history may retain
claimant/token/start audit while clearing lease; it must validate as history and
count zero, not raise incomplete claim. Conversely an active claimed state with
missing/invalid lease/identity must fail closed. Do not simply ignore every null
lease or every non-claimed row: use the existing authoritative lifecycle validator
and its accepted state/claim combinations, including committed/attention states.
Any state that legitimately retains an effective live lease counts while valid;
finalized/released historical claims do not. Preserve existing records, no schema
rewrite, cleanup/backfill or fabricated lease. The shared count feeds both atomic
fencing and refresh inspection; no divergent history validator in service.

REFRESH-CLAIM-TIME-ORDERING: capture now once per transaction/inspection, parse once
to an aware datetime, compare parsed aware lease instants using timezone-aware
ordering (UTC normalization permitted). Do not compare raw ISO strings. Z, offsets
and differing fractional precision representing the same instant must compare
equal; expiry == now is expired. Naive/malformed timestamps remain invalid, as do
invalid live claim shapes. Validate start/lease facts according to the existing
lifecycle contract; do not invent new historical timestamp constraints. No
wall-clock reads inside the per-row loop. Preserve valid future claim rejection
and expired/history acceptance across supported predecessor/current schemas.

### Ownership, state and tests

One bounded continuation, refresh-manager-claim-proof. Same workspace and preserved
candidate, single mutable Astra Low recovery owner; no separate contract precursor,
new context/worktree or concurrent mutable lane. Public/persisted contract impact
none: one PRIVATE result Protocol gains stdout; exact manager query and parser
remain service internals; ledger uses existing decision states/time types/tables.
No module/public command/table/schema/transport/lifecycle authority is added.

| Path | Action/responsibility |
| --- | --- |
| src/codex_flow/service.py | modify: existing private result/runner seam plus joint stopped-manager observation and its proof-boundary consumers |
| src/codex_flow/ledger.py | modify: state-aware claim counting/validation and instant comparison only |
| tests/test_service_lifecycle.py | modify: structural fake results, exact show grammar, MainPID/ActiveState and every stopped-proof public lifecycle regression |
| tests/test_ledger_integrity.py | modify: public claim lifecycle history and aware instant/fencing/inspection regressions |
| tests/test_plugin_installation.py | modify: fake result contract only where used by actual disposable service flow; retain installer behavior |

Everything else preserved, including accepted executable identity producer and
helpers, socket confinement/current recovery/v18 mechanisms, constructor/migration
contract, installer production code, contracts/schemas/test partitions, routing/
AGENTS, SDK/IPC, canonical plan (executor never stages it), untracked TUI, live
service/ledger/tool/plugin/global/auth and all downstream state. Dependency remains
service -> ledger/OS/existing runner; ledger owns SQL/lifecycle validation. New
artifact budget zero, private helper budget two functions above, no new classes.

Short falsification checkpoint: reproduce review probes through public refresh
using injected structural result objects, with inactive + nonzero MainPID (including
a live PID different from recorded authority) and valid inactive + zero control.
The former cannot migrate/start/restore or report safe retry, the latter succeeds
only with all other proofs. Assert exact query/selected unit and one response per
observation. Test missing, duplicate, unexpected, malformed, bytes/nonstring output,
noninteger/bool status, failed/transitional states and nonzero query exit. Inject
manager change after prior is-active/wait result and before final effect; prove
final show blocks. Cover current re-entry and legacy positive-absence/rollback
without reopening accepted guarantees.

Ledger checkpoint: construct actual public claim -> submit/acknowledge or release
history that retains audit and clears lease, then call public refresh fence and
inspection: valid history succeeds/counts zero. Active human/model lease blocks;
malformed active claim rejects. Parameterize equal instants, earlier/later instants
with positive/negative offsets, Z and alternate fractional precision (lexical order
opposite chronological order), equality at captured now, naive/malformed strings.
Use real disposable ledgers and existing public lifecycle, no production SQL or
live service operations. Preserve all three finding IDs in results.

Then run focused tests and affected service/ledger/installer partitions; full
make check is the explicit final gate for this shared seam, with no repeated broad
run absent changed bytes/failure. Inspect full and staged diff, git diff --cached
--check and make one coherent successor commit on five owned paths, no amend or
discard. Reuse accepted evidence for unchanged closures. After delivery controller
requests ONLY ONE focused Sol Medium review of these three blockers, per latest
specific routing; no new Luna/Astra duplicate reviews unless new material ambiguity
requires explicit controller escalation. The focused review must close all three,
including blocking P2, with no promotion blockers. Prior broader review requirements
are superseded only for this bounded continuation. No installer retry before that
acceptance and controller decision.

Readiness READY after controller accepts plan; next owner Astra Low bounded
implementation recovery. This private contract decision is Astra Medium planning;
Sol Medium focused acceptance follows delivery. Internal CONTINUE_WITH_REPLAN is
nonterminal; no user decision required. Parent controller
01a076b2-e4c5-7bc0-b5b1-d6ca5a7be818 host local alone owns dispatch/review/installation.

## Historical capsule — refresh-manager-claim-proof

```python
ModelFacingCapsule(
    schema_version=1,
    objective="Close REFRESH-CLAIM-HISTORY-REJECTED, REFRESH-MANAGER-LIVE-PID-UNPROVEN and promotion-blocking REFRESH-CLAIM-TIME-ORDERING at exact 4572917e23e40e4f6df0b50ad40c073689f294b6 while preserving accepted refresh closures.",
    decomposition=(
        "Extend the existing private structural manager result with textual stdout, retain integer _run_manager interface and parse one exact-unit ActiveState/MainPID show response at every stopped-proof boundary.",
        "Repair existing ledger claim count with state-aware retained-history validation and aware instant comparison using one captured now.",
        "Reproduce public manager/history/timestamp probes, run focused/affected/full gates and commit only the five owned paths without amending reviewed history.",
    ),
    acceptance_modes=(AcceptanceMode.ARCHITECTURE,),
    acceptance_criteria=(
        "No stopped-owner effect relies on is-active status 3 alone: one bound show response proves inactive and MainPID zero, and missing/malformed/duplicate/unexpected/transitional/nonzero-PID observations reject.",
        "Valid terminal/released claim history with retained audit and cleared lease is accepted/counts zero; active malformed claims reject and live human/model leases block atomic fence and inspection.",
        "Equivalent timestamp instants compare equally and lease expiry ordering is timezone-aware with one transaction now; naive/malformed timestamps reject.",
        "Accepted executable/socket/current-recovery/v18 proof remains intact, all focused/affected/full gates pass and one exact successor is delivered for only one focused Sol Medium review of all three blockers.",
    ),
    mutable_surfaces=("src/codex_flow/service.py", "src/codex_flow/ledger.py", "tests/test_service_lifecycle.py", "tests/test_ledger_integrity.py", "tests/test_plugin_installation.py"),
    protected_surfaces=("docs/reviews/peer-thread-workflow.md", "docs/reviews/conversational-tui", "src/codex_flow/harness.py", "src/codex_flow/domain.py", "src/codex_flow/contracts.py", "src/codex_flow/ipc.py", "scripts/install_personal_workflow_skills.py", "schemas", "config/test-partitions.toml", "workflow.toml", "AGENTS.md", "pyproject.toml", "uv.lock"),
    authorities=(ModelAuthority(AcceptanceMode.ARCHITECTURE, RoleId("architecture-reviewer")),),
    prompt="Use recover-milestone as the bounded Astra Low recovery owner for refresh-manager-claim-proof at 4572917e23e40e4f6df0b50ad40c073689f294b6 in the same workspace. Follow Current bounded recovery and exact private result/output grammar, ledger history and timezone decisions. Modify only five owned paths; preserve all accepted executable/socket/current-recovery/v18 mechanisms and untracked/dirty plan/TUI. No new public command/module/table/schema/transport/lifecycle owner or concrete CompletedProcess assumption. Reproduce all three public probes before repair, then focused/affected/full make check, self-review/staged diff/git diff --cached --check and one successor commit without amend. No live install/service/ledger/global/auth/provider/SDK/downstream mutation, subagents/peers/reviews/workflow-control or installer retry. Return exact candidate and three causal outcomes with one focused Sol Medium review pending; parent alone launches it. Material new ambiguity returns to planning; no automatic escalation or unsafe fallback.",
    recovery_policy="completion_biased",
)
```

## Prior repair — refresh identity, claim and re-entry proof

REPLAN_NOTICE, planning-only at exact candidate
9fa58171736e12d34f1d540af69baf30d21cc119. Preserve its history and accepted
predecessor fence 9b888f2ca0427445cd3ff690714cb116dfc1709a; no amend/discard.
Parent reports independent Luna correctness/integration ACCEPT, P0/P1/P2=0,
65 focused tests and full make check 984 passed. Combined Astra Medium
architecture/security review REJECT: four P1 and one P2, all five explicitly
promotion-blocking. Objective acceptance is valid for that candidate but cannot
substitute for architecture acceptance or acceptance of a later repair.

The live fenced service/ledger and partial installation remain untouched. No
installer retry is authorized until exact repair reviews accept. Only this plan
was changed in planning; dirty canonical content and untracked conversational-tui
artifacts are retained. This section supersedes earlier installation readiness.

### One bounded successor and frozen contract decisions

One coherent repair, refresh-authority-proof, owns the two existing production
modules and their boundary tests. A shared-contract precursor was considered:
ledger claim/authority inspection alone would not close the service guarantee,
while the service cannot be accepted without it. Keep one owner and one commit
for this small coupled transition; no fan-out, extra worktree or stub milestone.
Program dependency is repair -> both independent reviews -> controller installation
reconciliation, with state-authority and acceptance serial edges. Approval and
app-load activation stay blocked; TUI remains paused.

REFRESH-EXECUTABLE-PROOF (P1): preserve harness authority executable_digest meaning
as SHA-256 of bytes of Path(sys.executable).resolve(), verified in current producer
harness.py. Do NOT change it to launcher bytes or compare unlike identities.
service.py adds one private installed-command identity helper: validate exact
installed unit/launcher path, regular-file identity and unchanged launcher bytes,
then resolve the installed Python console launcher's absolute interpreter from
its shebang. Accept the installed canonical direct absolute-Python shebang form;
reject env/PATH lookup, shell wrappers, malformed/relative/ambiguous interpreter
forms rather than guess or execute them. Interpreter symlinks resolve as the
producer resolves them; require final regular executable and stable bytes/identity
through hashing. Keep a separate invocation-local launcher digest and resolved
interpreter digest. Recheck launcher/interpreter identities before start and health;
compare replacement executable_digest to interpreter digest. Also bind live
replacement PID/birth and, using existing Linux process inspection boundary,
resolved /proc/<pid>/exe identity to that interpreter, rechecking birth identity
around the read. The digest is interpreter identity, not full package attestation;
unit/launcher integrity, native version/profile checks remain separate proofs.
Old predecessor digest may differ after tool replacement; never require it to
match new bytes. No producer/persisted row change, new schema or arbitrary command
execution is needed. Tests use a canonical temporary Python launcher, not a shell
script masquerading as the installed command. Unsupported launcher format fails
with an actionable ServiceRefreshFailed; no fallback producer rewrite.

REFRESH-LIVE-CONTROLLER-CLAIM (P1): extend the existing ledger-owned
_raise_if_active_harness_children_in_transaction predicate with live
controller_decisions claims. Use the same criterion already present in
refresh_state_inspection: claimant_kind/id present and claim_lease_expires_at
strictly after the single captured now. Human and model claims both count even
without an active generation/queue/worker. Preserve strict schema validation;
malformed claim/lease facts fail closed, never parse as expired. Both predecessor
and current fence methods invoke this shared predicate atomically before writing
the bit, including an already-fenced row. Failed fencing does not change the row.
Expired valid claims do not block. No service raw SQL, new claim service or table.

REFRESH-PREDECESSOR-DRIFT (P1): extend existing Ledger.refresh_state_inspection
rather than adding an inspection wrapper. Preserve its existing JSON keys; support
validated allow_legacy schema 18/19/20 and current 21 using the existing authority
table selector. Read authority, children and live claims in one transaction with
one now; do not return artificial zero counts when authority is absent. Service
validates exact predecessor identity (repo/root/version/PID/birth/epoch/nonce/
executable identity/fence) against the captured fence and requires zero children/
claims immediately before migration. Liveness/unit/socket checks follow that
fresh snapshot; refresh an observed pre-stop lease renewal before capturing the
final stopped row, but never rebaseline a changed owner, epoch or fence. Missing
row, changed owner or live child/claim rejects without migration/start.

Close the legacy-opener-to-migrating-opener race without another state authority:
add optional keyword expected_refresh_authority to existing Ledger constructor,
accepted only with migrate=True for this explicit service handoff. It carries the
final stopped/fenced authority snapshot already held by service (existing closed
JSON shape, no new public record); omitted preserves existing callers. Validate
it at the existing migration fence boundary inside the transaction that performs
the first schema change, compare exact authority row, reject active children and
live claims there again. Do not keep comparison solely before BEGIN. Later steps
in the same migration chain retain the validated transaction guarantee; if existing
migration stages use separate transactions, revalidate carried authority at every
such boundary without changing legitimate table-rename identity. This optional
in-process API is the only new public parameter, no persisted format change.
Retain absent-authority legacy behavior for ordinary existing migrate callers;
the explicit service handoff always supplies a required non-null authority.
A raced successor row with fence=1 still fails exact identity comparison.

REFRESH-REENTRY-SOCKET-PROOF (P1): unify the stopped-current-owner proof used before
migration, current-schema re-entry and final start: fresh atomic authority/children/
claims inspection, fence=1, exact captured owner, birth-bound death, unit inactive,
no manager-owned live PID, matching retained unit/launcher, and absence by lexists
of BOTH harness.sock and unexpected supervisor.sock. Any socket entry (regular,
valid socket, dangling link) blocks without unlinking. A live owner must use the
existing authenticated endpoint and complete shutdown proof; an absent endpoint
never proves death. A new invocation cannot reuse a prior in-memory
possible-replacement flag as proof. Reconstruct these observations from retained
state every time; unknown/ambiguous liveness, active manager or residual socket
keeps failure/deferred and no start. Repeat immediately before start after reload/
credential import so a late entry/claim/owner change cannot slip through. Do not
require full isolation against arbitrary same-account writes after the check;
retain the explicit lifecycle boundary, no hidden socket cleanup or retry loop.

REFRESH-CURRENT-RECOVERY-USES-LEGACY-CHECK (P2, promotion_blocking=true): after an
exact acquired unhealthy current replacement is fenced and stopped, run the same
current stopped-owner/unit/socket proof. Retain exact CURRENT unit bytes/mode,
launcher/interpreter identity and migrated schema. Return failed but safely
retryable on a later explicit invocation; never compare to null legacy_unit_raw,
restore a legacy unit, downgrade schema or clear fence. Branch to existing v18
legacy restoration only for proven legacy topology; retain every accepted v18
compensation and uncertain-start exclusion. A current unhealthy owner with residual
socket or changed unit remains unresolved, not safely retryable. No automatic
second start is authorized by this repair.

### State, errors and boundaries

One service lifecycle authority and existing authenticated shutdown transport.
Supported current handoff: validated installed command -> atomic fence/children/
claims -> authenticated old-owner shutdown -> exact stopped proof -> transactional
expected-authority migration -> current unit/reload/import -> repeated stopped
proof -> start -> exact new live executable/PID/birth/epoch/fence-clear health.
Current-schema stopped re-entry skips migration but none of its departure proofs.
Failure before migration retains old schema/fence; committed migration remains
forward-only. Any uncertain start preserves current bytes/schema and forbids
rollback/automatic repeat. Exact acquired unhealthy replacement may only fence,
stop and prove retained current state as above. Historical rows survive all paths.

Use existing HarnessRefreshBlocked -> ServiceRefreshDeferred for active children/
claims and existing LedgerError/ServiceRefreshFailed for malformed/stale/missing
proof. Keep error stage/identity secret-free; no credentials, command output dumps
or inferred success. No topology/permission/transport/global changes. No service
SQL, new Protocol/class/table/schema/public command or durable recovery artifact.

### Exact map, checks and local closure

| Path | Action/responsibility and allowed dependency direction |
| --- | --- |
| src/codex_flow/service.py | modify: private launcher/interpreter identity and shared stopped-current-owner proof; existing ledger methods, OS/process and manager/IPC seams only |
| src/codex_flow/ledger.py | modify: existing atomic active-child predicate includes claims; existing inspection supports predecessors; optional expected authority guarded within migration transaction |
| tests/test_service_lifecycle.py | modify: public refresh probes for all five findings and legacy/current parity |
| tests/test_ledger_integrity.py | modify: real v18/v19/v20/v21 atomic fence/inspection, exact migration authority races and live/expired claims |
| tests/test_plugin_installation.py | modify: installed canonical launcher/partial service flow proof and failure withholding cache/plugin |

Every other path is protected, particularly harness.py producer, domain/contracts,
schemas, installer production code, runtime_evidence.py, worktree/approval/TUI,
Makefile/dependencies/test partitions, AGENTS/routing/global/auth, canonical plan
(executor may not stage it), untracked conversational-tui, live service/ledger and
all app-load/downstream state. One implementation owner, five allowed paths, no
new files, one coherent successor local commit after staged/full diff inspection
and git diff --cached --check. If the exact launcher form or migration transaction
layout disproves the frozen mechanism, return its bounded evidence before expanding
ownership or changing persisted producer semantics; not a generic user decision.

Start with the review's five reproduced disposable probe scenarios, recreated
through public entrypoints and real temporary ledgers; parent supplies retained
probe artifacts if needed. No live probe rerun. Short falsification checkpoint:
(1) wrong replacement interpreter digest fails health, exact correct digest passes;
(2) changed fenced predecessor/claim injected after stop and before migration,
including between opener close and migration transaction, yields no migration/start;
(3) current schema dead/fenced re-entry with a regular OR valid harness.sock yields
no start; (4) live human/model claim with no active child blocks fence without
mutation; (5) acquired unhealthy current replacement stops with no socket and
retained current unit, then fresh explicit retry succeeds without legacy checks.
An unresolved valid socket remains blocked. Assert observable post-state, not only
exceptions. These tests must falsify candidate 9fa5817 before asserting repair.

Then cover source/current interpreter drift, launcher drift after capture, wrong
/proc executable/PID reuse, malformed shebang, exact parent/root/version mismatch,
expired vs future claim, missing authority, pre/post-migration failures and
uncertain replacement start. Use fake process/manager/shutdown seams and real
filesystem sockets/temporary ledgers; no provider/SDK/install/global process.
Retain v18 legacy compensation and current forward-only behavior tests. Run
focused tests, affected service/installer/ledger partitions and full make check
because a shared ledger constructor/migration boundary changes. Reuse prior
successful checks only for unchanged bytes/dependencies, no claim that 984 tests
accept new bytes. Independent Luna XHigh objective review and explicitly escalated
Astra Medium combined architecture/security review must bind the final successor,
close all five finding IDs including blocking P2, P0/P1=0 and no open promotion
blockers. Sol Medium is not substituted for this named cross-boundary review;
High is unavailable absent new explicit authorization.

Readiness: refresh-authority-proof READY after parent accepts this bounded plan;
next owner same Luna XHigh implementation lane at exact 9fa5817 predecessor, with
actual plan-commit tip supplied by controller if advanced. Parent alone owns
reviews and eventual installation retry. No installer retry from this notice.
Planning Astra Medium; code/objective Luna XHigh; ordinary visual/recovery Astra
Low with Medium explicit escalation as here. No routing file changes.

## Superseded capsule — refresh-authority-proof

```python
ModelFacingCapsule(
    schema_version=1,
    objective="Repair five promotion-blocking refresh findings at 9fa58171736e12d34f1d540af69baf30d21cc119: executable identity, predecessor drift, re-entry socket proof, live controller claims and current recovery legacy-check leakage.",
    decomposition=(
        "Preserve interpreter-digest producer semantics; bind validated installed launcher to resolved interpreter and replacement process identity at health.",
        "Extend existing ledger atomic children/claim and refresh inspection boundaries, and guard migration transaction against the exact final stopped authority.",
        "Reuse one current-topology stopped-owner/unit/socket proof at migration, re-entry, pre-start and unhealthy replacement recovery; preserve legacy v18 compensation and forward-only current state.",
        "Reproduce the five disposable review probes, pass affected gates/full make check, self-review and commit exactly the five owned paths.",
    ),
    acceptance_modes=(AcceptanceMode.OBJECTIVE, AcceptanceMode.ARCHITECTURE),
    acceptance_criteria=(
        "Replacement health compares the existing interpreter digest to the installed launcher's resolved interpreter and live PID/birth executable identity; wrong/drifted identity fails without producer-schema changes.",
        "Fresh atomic stopped-authority/children/claims proof and migration transaction expected-authority comparison reject changed owners and live claims before schema mutation/start.",
        "Current-schema re-entry with any unresolved socket or uncertain live owner cannot start; safe re-entry reconstructs retained facts instead of invocation-local history.",
        "Current unhealthy replacement recovery proves socket absence and retained current unit without legacy comparison/downgrade; v18 and uncertain-start guarantees remain unchanged.",
        "All five causal probes and affected/full gates pass, including blocking P2; one exact successor commit is delivered with Luna objective and explicit Astra Medium combined review pending, no installer retry.",
    ),
    mutable_surfaces=("src/codex_flow/service.py", "src/codex_flow/ledger.py", "tests/test_service_lifecycle.py", "tests/test_ledger_integrity.py", "tests/test_plugin_installation.py"),
    protected_surfaces=("docs/reviews/peer-thread-workflow.md", "docs/reviews/conversational-tui", "src/codex_flow/harness.py", "src/codex_flow/domain.py", "src/codex_flow/contracts.py", "src/codex_flow/ipc.py", "scripts/install_personal_workflow_skills.py", "schemas", "config/test-partitions.toml", "workflow.toml", "AGENTS.md", "pyproject.toml", "uv.lock"),
    ...
    prompt="Use execute-milestone as Luna XHigh leaf for refresh-authority-proof at exact 9fa58171736e12d34f1d540af69baf30d21cc119 in the existing lane. Follow Current repair and its frozen five-path map/contract decisions. Preserve accepted 9b888f2 fence, history and all unowned bytes. Reproduce all five architecture probes first; fix interpreter-vs-launcher proof without producer changes, atomic claims/expected migration authority, and one current stopped-state/socket proof for re-entry and unhealthy recovery. Keep v18 compensation, no current downgrade/unsafe retry and no service SQL. New public API limited to existing Ledger constructor optional expected_refresh_authority; no new modules/types/tables/schema/commands. Return exact counterexample if launcher or migration layout requires a material map update. No live service/install/ledger/global/auth/provider/SDK/downstream mutation, subagents/peers/reviews/workflow-control/worktree changes or installer retry. Preserve dirty plan/TUI. Run affected gates/full make check, self-review and inspect exact staged diff/git diff --cached --check, create one successor commit without amend. Return candidate/evidence with all five causal reviews and objective successor acceptance pending; parent owns lifecycle.",
    recovery_policy="completion_biased",
)
```

## Prior installation recovery — same-topology harness schema refresh

Diagnosis: architecture_replan, bounded medium remaining work. Exact source HEAD
9b888f2ca0427445cd3ff690714cb116dfc1709a on agent/python-sdk-controller in
/home/adam/personal-workflow-skills.worktrees/python-sdk-controller. The canonical
plan e0c8afa, routing 95ac631 and predecessor fence 9b888f2 are accepted as supplied
by parent. The fence repair is not reopened. This section supersedes installation
readiness below; code acceptance does not mean installation completion.

Current partial installation, supplied by parent (not independently read from
live service/ledger in this planning turn): first installer attempt stopped before
tool installation because fence arming supported only v18. After accepted fence
repair, the second canonical installer passed preflight, armed v20
harness_authority.requested_shutdown=1, replaced the shared controller tool, then
failed with "service refresh only supports the installed schema-v18 predecessor
handoff". Existing harness PID 2388572 remains reported running with schema v20
and fence=1. No schema migration occurred. Plugin/cache were not updated. Preserve
this fenced owner, installed tool, all historical rows and app-load service/state;
never report source/cache/runtime parity or installed routing yet. PID is an
observation for recovery context, not a hardcoded implementation identity.

Read-only service.py inspection confirms migration_required is assigned only for
schema 18; both compatibility and supplied-Ledger branches reject newer migrations.
Every migration branch then chooses _legacy_supervisor_unit, supervisor.sock,
legacy unit staging and restore_legacy_pair requiring schema 18. The already
accepted ledger.arm_predecessor_refresh_fence/predecessor_refresh_fenced support
18->supervisor_authority and 19/20->harness_authority. Installer order is fence,
shared-tool replacement, refresh; old running process and new launcher bytes can
therefore coexist intentionally while the old schema is fenced. No guarantee
must change; no user decision is needed. This is explicit Astra Medium escalation
after a service architecture assumption failed, not ordinary recovery routing.

### Frozen minimal design and authority

Keep refresh_with_credential as sole public refresh entrypoint. Separate existing
schema migration decision from topology decision using local primitive values:
predecessor_schema_version, migration_required, legacy_topology_migration. The
last is true only for supported schema 18 with its exact supervisor unit. Schemas
19 and 20 require migration but retain current harness unit/harness.sock. Current
schema 21 follows ordinary harness refresh. Unknown/incompatible/corrupt schemas
fail before lifecycle changes. No new public type, table, command, registry,
transport, dependency or durable recovery file. Private helpers inside service.py
may compress selection and shared stop/proof phases; do not fork a whole second
refresh runner. Keep the tested legacy interrupted-topology path separate by
positive exact unit identity, not by current schema number alone.

For every required migration, open Ledger(..., allow_legacy=True) and retain that
schema until old owner stop/death/socket proof. Never let preflight auto-migrate.
For a supplied legacy Ledger, inspect its actual supported schema and preserve
existing caller/owned-handle close semantics. Use accepted predecessor fencing
for 18/19/20, current harness fencing only for current schema. Fencing repeats
must recheck active dispatches, worker leases, controller generations and claims;
an existing fence=1 is not permission to ignore active children or changed owner.
Use the existing idempotent ledger fence operation to revalidate, never clear and
rearm the fence. Malformed/absent/multiple authority or invalid fence fails closed.

Bind physical repository/state-root, exact installed unit path/bytes/command and
configuration/profile authority; capture predecessor PID/birth/epoch/fence before
shutdown and require unchanged identity until its departure is proven. Revalidate
unit and authority at stop/migration boundaries. Keep old running executable
identity distinct from installed replacement launcher identity: the current partial
state legitimately has old-process bytes and new on-disk tool. Do not require
old executable digest == replacement digest or synthesize the old digest from the
new file. Preserve existing version/profile compatibility checks. Replacement
health must bind the new authority executable digest to the actual installed
replacement command using the existing executable identity mechanism, plus exact
repo/root/version, live PID/birth, active unit, greater epoch and fence=0. No global
configuration/auth scan or new executable provenance store is introduced.

### State transitions and failure policy

| State/observation | Required action and retained result |
| --- | --- |
| Supported v19/v20, matching current unit, old owner live, fence 0 or 1 | Check children/claims and capture exact authority; idempotently fence via predecessor API. Authenticate shutdown through harness.sock, never supervisor.sock. |
| Authenticated shutdown accepted | Wait for exact old PID/birth death and current unit inactivity within deadline. Use existing manager stop only for the exact validated unit if needed; no kill/foreign service action. |
| Old owner already stopped/fenced after interruption | Require exact retained identity, no active children/claims, unit inactive, old process dead and owned socket absent; do not require a live endpoint or resend shutdown. |
| Predecessor live but socket missing/unsafe/unresponsive or identity changed | Fail/defer with fence retained; do not migrate, guess death, unlink socket or start replacement. |
| Stop proof complete | Prove harness.sock absent (and no unexpected legacy socket), no active manager process, unchanged authority/fence and safe matching unit immediately before migrating. Close legacy opener and use existing Ledger(..., migrate=True) once. |
| Migration transaction fails/rolls back | Retain fenced v19/v20 and stopped current unit/tool; re-read compatibility read-only to identify old vs committed current schema. No legacy pair restoration or old-tool restart. |
| Migration commits, later open/staging/reload/import fails before start | Retain current schema and current harness tool/unit, with fence set and no live owner. Next invocation recognizes current-schema fenced/dead state and continues forward. Never restore schema 18 or downgrade. |
| Replacement start requested, raised, timed out or nonzero | Set existing monotonic possible-replacement guard BEFORE request. Retain current schema/tool/unit; uncertain state never enables rollback, second start or automatic retry. Report exact failure for controller reconciliation. |
| Exact newer replacement acquired but unhealthy | Reuse existing validated replacement fencing (0->1 once, 1 reused), exact stop/death/socket proof; leave current schema/tool/unit fenced/stopped for later explicit retry. No schema/unit downgrade. |
| Replacement missing, ambiguous, malformed, live/unknown or changed identity | No restoration, migration replay, hidden service or second start. Preserve current bytes and report failure/uncertainty. |
| Exact replacement healthy | Current schema, unit active, new executable identity, live new PID/birth, epoch strictly advanced and fence cleared by normal lease acquisition. Return refreshed success; installer then owns plugin/cache continuation. |

Schema migration and topology replacement must remain independent: only the legacy
18 supervisor->harness path may stage/restore a legacy unit and use its existing
v18 compensation proof. Same-topology 19/20->21 NEVER calls
restore_legacy_pair, restore_legacy_unit or legacy schema compensation, even after
an error. Transactional rollback performed by the ledger itself is distinct from
service-initiated downgrade. Shared rollback_failed_replacement may retain its
fence/stop/death logic but must select topology-specific post-death behavior:
legacy policy unchanged, current harness retains current bytes. No rollback after
uncertain replacement is introduced. Do not broaden historical migration rollback
code or weaken _assert_migration_fenced.

Restart reconciliation uses existing schema/unit/authority/manager/process/socket
facts only. Current schema + exact fenced dead owner + no children/socket is a
forward retryable state. Current schema + live healthy unfenced owner is an
ordinary refresh, never the stale predecessor; keep ordinary behavior. Current
schema + uncertain/live attempted replacement must be reconciled before another
start. With no durable evidence sufficient to distinguish an uncertain launch,
return truthful ServiceRefreshFailed/Deferred and smallest missing observation;
never infer success or retry from elapsed time. No new recovery state authority.

ServiceRefreshDeferred remains for active children/claims or an exact owner still
transitioning within the supported bounded lifecycle. ServiceRefreshFailed covers
unsupported/malformed identity, timeout, command/migration/health failure or missing
proof. Error reporting includes stage/schema/topology without credentials. Preserve
credential import cleanup on failure and existing secret-free output. Do not
manually clear a durable fence to make installation pass.

### Exact ownership, tests and bounded checkpoints

Next mutable owner: one Luna XHigh leaf in the same workspace at
9b888f2ca0427445cd3ff690714cb116dfc1709a (parent may commit this plan first and must
supply actual successor base). Only these paths are authorized:

| Path | Action and responsibility |
| --- | --- |
| src/codex_flow/service.py | modify: classify schema versus topology; reuse fenced shutdown/stop/migration/start phases and topology-appropriate recovery; existing ledger/IPC/manager dependencies only |
| tests/test_service_lifecycle.py | modify: real disposable v19/v20/current ledgers with fake manager/liveness/shutdown seams, exact interrupted partial and all failure/retry states |
| tests/test_plugin_installation.py | modify: canonical installer order/partial-state boundaries, replaced tool before refresh and plugin/cache withheld on failure; provider-free runner only |

Preserve ledger.py and accepted fence/migrations, installer production code,
service public signature/types, contracts/schemas, dependencies, test partitions,
routing/AGENTS/plugin/cache, global/auth state, canonical plan and untracked TUI,
all downstream/app-load bytes and live runtime. No new files/durable artifacts.
If the first discriminating checkpoint proves an existing ledger observation
boundary cannot support safe current-schema inspection, return the exact missing
capability for bounded planning update before editing ledger; do not access raw SQL
from service or weaken proof. Tests may use existing fixture setup patterns only
in disposable roots; production never patches schema/version metadata.

Checkpoint 1: construct a real disposable v20 ledger, current harness unit and
live exact old authority with fence=1 while launcher already contains replacement
bytes. Call refresh_with_credential through public entrypoint. Record event order:
active-child proof -> current harness.sock authenticated shutdown -> old process
and unit/socket absence -> migrating opener -> replacement start -> exact newer
healthy authority/fence=0. Fail if any supervisor path, v18 restore or migration
before stop occurs. Add fence=0 and v19 variants plus active-child rejection.
This immediately falsifies the old topology assumption; stop and return an exact
counterexample if it cannot pass within the frozen authority boundaries.

Checkpoint 2: failures before shutdown, shutdown/stop/socket timeout, unit staging,
transactional migration failure, committed migration followed by open failure,
daemon-reload/import, start exception/nonzero/timeout, missing/malformed/drifted
health and acquired-unhealthy replacement. Assert schema/tool/unit/fence/liveness
post-state, secret-free error and no duplicate start. Prove interrupted re-entry
both before migration and after committed migration with stopped fenced owner.
Prove current v21 healthy ordinary refresh and all accepted v18 topology/
interrupted/uncertain-start restoration semantics remain unchanged. Use a real
temporary ledger migration, not only compatibility-dictionary mocks.

Checkpoint 3: canonical installer test captures both failure boundaries and one
successful fake runtime flow: shared-tool install is allowed only after fence;
refresh failure leaves plugin/cache update unexecuted; accepted refresh permits
later plugin/cache steps. It must exercise the actual service entrypoint within
disposable context for current topology, not merely assert command strings.
No live installer/service/provider/SDK process may run in these tests.

After focused checks, run affected service/installer/ledger integration partitions
and full make check (shared service/migration gate). Preserve accepted unchanged
checks; broaden/repeat only for changed dependencies or failures. Self-review full
diff, stage exact three paths, inspect staged diff and git diff --cached --check,
make one coherent successor commit, never amend accepted fence history. Require
independent Luna XHigh objective and Astra Medium combined architecture/security
review binding exact candidate with P0/P1=0 and no open promotion blockers. Parent
alone owns later installer retry/live reconciliation and source/tool/plugin/cache/
service/schema parity proof. Repair delivery or tests alone are not installation.

### Routing and program readiness

Planning Astra Medium; ordinary code/objective Luna XHigh. Visual implementation,
visual review and ordinary recovery default Astra Low; this architecture recovery
is explicitly escalated to Astra Medium. Accepted routing commit 95ac631 remains
unchanged and installation incomplete. No routing/global update belongs to this
repair. Same-topology refresh repair -> independent acceptance -> controller
installation reconciliation is serial for service/state authority and acceptance.
Approval capability, app-load program activation and conversational TUI remain
blocked/deferred as previously recorded; no downstream operation is authorized.
Recovery status CONTINUE, user decision required no.

## Superseded capsule — current-harness-schema-refresh

```python
ModelFacingCapsule(
    schema_version=1,
    objective="Repair current-harness-schema-refresh: safely refresh supported v19/v20 harness ledgers to v21 in the same unit/socket topology, including the already-fenced state after shared tool replacement, while preserving legacy v18 handoff and uncertain-start safety.",
    decomposition=(
        "Separate supported schema migration from legacy topology selection inside the existing service refresh entrypoint; use the accepted predecessor fence and nonmigrating opener.",
        "Reuse exact authority/children/shutdown/stop/death/socket proof before migration, then start and verify current executable/new epoch/fence-clear authority.",
        "Keep current-topology failures forward-retryable and fenced without legacy restoration or downgrade; preserve existing legacy compensation and no rollback after uncertain start.",
        "Pass the three discriminating disposable service/installer checkpoints, affected partitions and full make check; self-review and commit only the three owned paths.",
    ),
    acceptance_modes=(AcceptanceMode.OBJECTIVE, AcceptanceMode.ARCHITECTURE),
    acceptance_criteria=(
        "A real disposable v20/v19 current-harness ledger, already fenced with new launcher bytes and old live owner, uses harness.sock and migrates only after exact old process/unit/socket departure, then proves healthy newer current authority.",
        "Active children, changed/missing authority, unsafe/stale sockets or uncertain liveness reject before migration/start; fence=1 does not bypass child or identity validation.",
        "Migration failures and post-commit failures preserve truthful schema/unit/tool/fence states; same-topology paths never restore v18/legacy units or downgrade, and uncertain replacement never causes rollback or automatic second start.",
        "Interrupted before/after-migration stopped-owner re-entry succeeds safely; current ordinary refresh and accepted legacy topology/rollback proof retain parity.",
        "Canonical installer with disposable service flow updates tool only after fence and withholds plugin/cache on refresh failure; full make check and affected gates pass with one exact owned successor commit and both independent reviews pending.",
    ),
    mutable_surfaces=("src/codex_flow/service.py", "tests/test_service_lifecycle.py", "tests/test_plugin_installation.py"),
    protected_surfaces=("docs/reviews/peer-thread-workflow.md", "docs/reviews/conversational-tui", "src/codex_flow/ledger.py", "scripts/install_personal_workflow_skills.py", "src/codex_flow/contracts.py", "src/codex_flow/domain.py", "src/codex_flow/ipc.py", "src/codex_flow/backends", "workflow.toml", "AGENTS.md", "config/test-partitions.toml", "pyproject.toml", "uv.lock"),
    authorities=(ModelAuthority(AcceptanceMode.OBJECTIVE, RoleId("code-reviewer")), ModelAuthority(AcceptanceMode.ARCHITECTURE, RoleId("architecture-reviewer"))),
    prompt="Use execute-milestone as one Luna XHigh leaf for current-harness-schema-refresh in the canonical plan. Preserve exact accepted 9b888f2 fence and implementation bytes outside three owned paths, canonical plan and untracked TUI. Separate migration_required from legacy topology; current v19/v20 uses current harness unit/socket and forward-only post-migration recovery, never v18 restore. Preserve exact child/authority/PID-birth/epoch/fence/unit/socket checks and monotonic uncertain-start guard. Test observed already-fenced old process/new launcher partial with real disposable ledgers and fake manager/shutdown/spawn, then failure/restart and installer public flow. If missing ledger capability prevents the first checkpoint return precise counterexample before changing ledger. No live install/service/process/ledger/global/auth/provider/SDK/downstream mutation, no manual fence clearing, no new files/schema/commands or routing changes. Run affected gates/full make check, self-review/stage exactly owned paths, git diff --cached --check, one coherent successor commit without amend. Return exact evidence and candidate with Luna objective/Astra Medium combined architecture-security reviews pending. No subagents, peer dispatch, review launch, workflow-control, installer retry or successor scheduling; parent owns lifecycle.",
    recovery_policy="completion_biased",
)
```

## Resolved first installation blocker — predecessor refresh fence
