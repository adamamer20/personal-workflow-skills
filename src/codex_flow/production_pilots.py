"""Integrated production pilots and legacy-retirement evidence."""
...
class PilotError(RuntimeError):
    """A production pilot could not satisfy its fixed contract."""


def build_visible_worker_capsule(*, state_root: Path, parent_run_id: RunId | str) -> ExecutionCapsule:
    """Build the bounded host-only visible-worker follow-up capsule."""

    checkout = state_root.resolve(strict=True)
    if not checkout.is_dir():
        raise PilotError("App-native pilot state root must be a checkout directory")
    ...
    def git_output(*arguments: str) -> str:
        ...
        if completed.returncode != 0 or not completed.stdout.strip():
            raise PilotError("unable to resolve App-native pilot Git facts")
        return completed.stdout.strip()

    if Path(git_output("rev-parse", "--show-toplevel")).resolve() != checkout:
        raise PilotError("App-native pilot requires the exact selected Git toplevel")
...
class LegacyRetirementDecisionRecord:
    """Closed legacy-retirement decision; it never authorizes cleanup by itself."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.closed is not True:
            raise ValueError("production legacy-retirement decision must be closed")
        if self.cleanup_authorized:
            raise ValueError("legacy-retirement evidence cannot authorize cleanup")
    ...
    for current, directories, files in os.walk(root, followlinks=False):
        ...
        for name in sorted(files):
            ...
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
                raise PilotError(f"pilot workspace contains an unsupported file: {relative}")
...
def _initialize_production_repository(root: Path, spec: ProductionPilotSpec) -> str:
    root.mkdir()
    _git(root, "init", "-q", "-b", "main")
    _git(root, "config", "user.email", "codex-flow@example.test")
    _git(root, "config", "user.name", "Codex Flow Production Pilot")
    _write(root / ".gitignore", ".codex-flow/\n__pycache__/\n")
    if spec.scale == "medium":
        _write(root / "pyproject.toml", "[project]\nname='production-medium-pilot'\nversion='0.0.0'\n")
        _write(
            root / "src/text_normalization/__init__.py",
            "from .slug import normalize_slug\n\n__all__ = ['normalize_slug']\n",
        )
        _write(
            root / "src/text_normalization/slug.py",
            '"""Text normalization boundary."""\n\n\ndef normalize_slug(value: str) -> str:\n'
            '    """Return a stable ASCII slug."""\n\n    raise NotImplementedError\n',
        )
        _write(
            root / "tests/test_text_normalization.py",
            "import sys\nimport unittest\nfrom pathlib import Path\n\n"
            "sys.path.insert(0, str(Path(__file__).parents[1] / 'src'))\n"
            "from text_normalization import normalize_slug\n\n\n"
        ...
        _write(root / "pyproject.toml", "[project]\nname='production-large-pilot'\nversion='0.0.0'\n")
        _write(
            root / "src/atomic_task_queue/__init__.py",
        ...
        _write(
            root / "src/atomic_task_queue/domain.py",
            '"""Typed queue domain."""\n\n# Implement this module without changing public names.\n',
        )
        _write(
            root / "src/atomic_task_queue/store.py",
            '"""Atomic JSON queue storage."""\n\n# Implement this module without changing public names.\n',
        )
        _write(
            root / "src/atomic_task_queue/service.py",
            '"""Queue use-cases."""\n\n# Implement this module without changing public names.\n',
        )
        _write(
            root / "tests/test_atomic_task_queue.py",
            "import json\nimport sys\nimport tempfile\nimport unittest\nfrom pathlib import Path\n\n"
            "sys.path.insert(0, str(Path(__file__).parents[1] / 'src'))\n"
            "from atomic_task_queue import Job, JobStatus, QueueService, RevisionConflict, StoreCorrupt, TaskStore\n\n\n"
    ...
    _git(root, "add", ".")
    _git(root, "commit", "-qm", f"production {spec.scale} pilot base")
    return _git(root, "rev-parse", "HEAD")


def production_pilot_specs() -> tuple[ProductionPilotSpec, ProductionPilotSpec]:
    validation = ("python3", "-B", "-m", "unittest", "discover", "-s", "tests", "-v")
    medium = ProductionPilotSpec(
        "medium",
        RunId("h6-medium-pilot"),
        MilestoneId("slug-normalization"),
        (
            "Implement src/text_normalization/slug.py only. normalize_slug must require str, lowercase ASCII alphanumeric "
            ...
            "Return status='completed', changed_files=['src/text_normalization/slug.py'], and validation='passed'."
        ),
        ("src/text_normalization/slug.py",),
        (".gitignore", "pyproject.toml", "src/text_normalization/__init__.py", "tests"),
        validation,
        ("src/text_normalization/slug.py",),
    )
    large = ProductionPilotSpec(
        "large",
        RunId("h6-large-pilot"),
        MilestoneId("atomic-queue"),
        (
            "Implement only src/atomic_task_queue/domain.py, store.py, and service.py as a cohesive typed standard-library "
        ...
        ("src/atomic_task_queue/domain.py", "src/atomic_task_queue/store.py", "src/atomic_task_queue/service.py"),
        (".gitignore", "pyproject.toml", "src/atomic_task_queue/__init__.py", "tests"),
        validation,
        ("src/atomic_task_queue/domain.py", "src/atomic_task_queue/service.py", "src/atomic_task_queue/store.py"),
    )
    return medium, large


def _run_workflow_control(root: Path, capsule_path: Path) -> tuple[subprocess.CompletedProcess[str], float]:
    executable = shutil.which("codex-flow")
    if executable is None:
        raise PilotError("the packaged codex-flow executable is unavailable")
...
def _review_authority_workspace(
    root: Path,
    *,
    mode: AcceptanceMode,
    role: RoleId,
    ...
    try:
        ...
        if payload is None:
            raise PilotError(f"{mode.value} reviewer returned no structured output")
        accepted = payload.get("accepted")
        raw_findings = payload.get("findings")
        if not isinstance(accepted, bool) or not isinstance(raw_findings, list):
            raise PilotError(f"{mode.value} reviewer returned an invalid typed result")
        seen: set[str] = set()
        ...
        for raw in raw_findings:
            if not isinstance(raw, dict):
                raise PilotError(f"{mode.value} reviewer finding is not an object")
            ...
            if (
                ...
                raise PilotError(f"{mode.value} reviewer finding is invalid")
            try:
                severity = Severity(severity_value)
            except ValueError as exc:
                raise PilotError(f"{mode.value} reviewer returned an unknown severity") from exc
            if blocking and severity not in {Severity.P0, Severity.P1}:
                raise PilotError(f"{mode.value} reviewer misclassified a lower-severity finding as blocking")
            seen.add(finding_id)
            finding_ids.append(finding_id)
            counts[severity.value] += 1
            blockers += int(blocking)
        if accepted != (blockers == 0):
            raise PilotError(f"{mode.value} reviewer acceptance contradicts its findings")
        after = _production_tree_digest(root)
        unchanged = before == after
        if not unchanged:
            raise PilotError(f"{mode.value} reviewer changed the read-only workspace")
        ...
        if thread is not None:
            try:
                ...
                raise PilotError(f"unable to archive {mode.value} review thread") from exc
        adapter.close()


def _review_authority_fact(review: ReviewAuthorityEvidence, revision: str) -> JsonObject:
    return {
...
def _raise_notification_failure() -> None:
    raise RuntimeError("injected production notification delivery failure")


def _run_production_pilot(spec: ProductionPilotSpec, config: WorkflowConfig, parent: Path) -> dict[str, Any]:
    root = parent / f"{spec.scale}-repository"
    base_sha = _initialize_production_repository(root, spec)
    ...
    completed, execution_seconds = _run_workflow_control(root, capsule_path)
    try:
        control_payload = json.loads(completed.stdout.strip())
    except (json.JSONDecodeError, TypeError) as exc:
        raise PilotError(f"{spec.scale} control returned invalid JSON") from exc
    if completed.returncode != 0 or not isinstance(control_payload, dict):
        raise PilotError(f"{spec.scale} control failed with exit {completed.returncode}")
    ...
    if not isinstance(result, dict) or result.get("status") != "completed":
        raise PilotError(f"{spec.scale} model result was not completed")
    changed = tuple(sorted(_git(root, "diff", "--name-only", base_sha).splitlines()))
    if changed != spec.expected_changed_paths:
        raise PilotError(f"{spec.scale} observable changed paths do not match the fixed capsule")
    validation = subprocess.run(spec.validation_argv, cwd=root, text=True, capture_output=True, check=False, timeout=60)
    if validation.returncode != 0:
        raise PilotError(f"{spec.scale} observable validation failed")
    revision = _production_tree_digest(root)
    ...
    try:
        ...
        if record.status is not ExecutionStatus.COMPLETED or record.result is None:
            raise PilotError(f"{spec.scale} durable controller result is incomplete")
        result_artifact = root / ".codex-flow" / "runs" / str(spec.run_id) / "execution.json"
        if not result_artifact.is_file():
            raise PilotError(f"{spec.scale} durable execution projection is missing")
        ...
        reviews: list[ReviewAuthorityEvidence] = []
        for mode, role_name in (
            ...
            review = _review_authority_workspace(
                root,
                mode=mode,
                role=route.role,
                model=route.model,
            ...
            controller.ledger.record_review_fact(
                spec.run_id,
                spec.milestone_id,
                phase=LifecyclePhase.REVIEW,
                kind="h6_independent_authority_review",
                data=_review_authority_fact(review, revision),
            )
        ...
        if blockers or any(not review.accepted for review in reviews):
            ...
            raise PilotError(f"{spec.scale} independent review returned P0/P1 promotion blockers")
        controller.ledger.record_review_transition(
            spec.run_id,
            spec.milestone_id,
        ...
        lifecycle = controller.ledger.review_lifecycle(spec.run_id, spec.milestone_id)
        review_threads = {review.thread_digest for review in reviews}
        if len(review_threads) != 2:
            raise PilotError(f"{spec.scale} review authorities did not use distinct threads")
        if dispatch_count != 1:
            raise PilotError(f"{spec.scale} controller recorded duplicate execution owners")
        if not terminal_authority_survived:
            raise PilotError("large notification recovery changed durable terminal authority")
        return {
...
def _build_compatibility_matrix(pilots: tuple[dict[str, Any], ...]) -> JsonObject:
...
def _build_pilot_comparison(pilots: tuple[dict[str, Any], ...]) -> JsonObject:
...
def run_production_pilots(*, config_path: Path) -> dict[str, Any]:
    """Run the fixed medium/large production suite and return sanitized evidence."""

    config = load_workflow_config(config_path)
    with tempfile.TemporaryDirectory(prefix="codex-flow-production-pilots-") as directory:
        parent = Path(directory)
        pilots = tuple(_run_production_pilot(spec, config, parent) for spec in production_pilot_specs())
    recovery_count = sum(
    ...
    if recovery_count != 1 or notification_failures != 1:
        raise PilotError("production suite must exercise exactly one bounded recovery and one notification failure")
    compatibility = _build_compatibility_matrix(pilots)
    comparison = _build_pilot_comparison(pilots)
...
def write_production_evidence(path: Path, evidence: dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
