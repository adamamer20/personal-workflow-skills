"""One-shot real SDK review workflow pilots."""
...
def run_review_pilot(*, model: str, effort: ReasoningEffort) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="codex-flow-review-pilot-") as directory:
        root = Path(directory) / "repository"
        root.mkdir()
        _git(root, "init", "-q")
        _git(root, "config", "user.email", "codex-flow@example.test")
        _git(root, "config", "user.name", "Codex Flow Review Pilot")
        target = root / "review_target.txt"
        target.write_text("before\n", encoding="utf-8")
        _git(root, "add", "review_target.txt")
        _git(root, "commit", "-qm", "pilot base")
        base = _git(root, "rev-parse", "HEAD")
        controller = Controller(root)
        ledger = controller.ledger
        ledger.create_run("review-pilot")
        ledger.create_milestone("review-pilot", "objective-review")
        ...
        try:
            ...
            def objective() -> dict[str, object]:
                ...
                observation = adapter.run_turn(
                    thread,
                    "In this disposable repository, write review_target.txt with exactly 'needs-repair' followed by one newline. "
                ...
                return {"changed_file": "review_target.txt", "turn_status": observation.status}
            ...
            def reviewer(revision_token: str, fresh: bool) -> ReviewResult:
                ...
                if content != "repaired\n":
                    finding = ReviewFinding(
                        ...
                        {"path": "review_target.txt", "content_sha256": hashlib.sha256(content.encode()).hexdigest()},
                        "review_target.txt contains the repaired marker",
                    )
            ...
            def repair(finding: ReviewFinding) -> RepairRecord:
                ...
                observation = adapter.run_turn(
                    thread,
                    "Repair the exact prior finding in this disposable repository: write review_target.txt with exactly "
                ...
                return RepairRecord(
                    "repair-1",
                    finding.finding_id,
                    DispatchId("review-pilot/objective-review/executor/1"),
                    "repair objective marker through the same SDK thread",
                    "repaired",
                )

            result = controller.review_workflow().run(
                "review-pilot",
                "objective-review",
                objective=objective,
                reviewer=reviewer,
            ...
            return {
                ...
                "lifecycle_kinds": [
                    record.kind for record in ledger.review_lifecycle("review-pilot", "objective-review")
                ],
                "review_count": len(result.review_ids),
                "repair_count": len(result.repair_ids),
                "same_owner_repair": True,
                "accepted_state": ledger.current_state("review-pilot", "objective-review").value,
            }
...
__all__ = ["run_multi_authority_review_pilot", "run_review_pilot"]


def run_multi_authority_review_pilot(*, model: str, effort: ReasoningEffort) -> dict[str, Any]:
    """Run exactly one bounded real SDK multi-authority review workflow.
    ...
    with tempfile.TemporaryDirectory(prefix="codex-flow-multi-authority-review-") as directory:
        root = Path(directory) / "repository"
        root.mkdir()
        _git(root, "init", "-q")
        _git(root, "config", "user.email", "codex-flow@example.test")
        _git(root, "config", "user.name", "Codex Flow Multi-Authority Review Pilot")
        target = root / "review_target.txt"
        target.write_text("before\n", encoding="utf-8")
        render = root / "review-rendered-evidence.svg"
        render.write_text('<svg xmlns="http://www.w3.org/2000/svg"><text>H4-B</text></svg>\n', encoding="utf-8")
        _git(root, "add", "review_target.txt", "review-rendered-evidence.svg")
        _git(root, "commit", "-qm", "pilot base")
        base = _git(root, "rev-parse", "HEAD")
        controller = Controller(root)
        ledger = controller.ledger
        ledger.create_run("multi-authority-review-pilot")
        ledger.create_milestone("multi-authority-review-pilot", "multi-authority-review")
        adapter = CodexSdkAdapter(
            CodexSdkConfig(
                model=model,
                reasoning_effort=effort,
        ...
        try:
            ...
            def objective() -> dict[str, object]:
                ...
                return {"changed_file": "review_target.txt", "turn_status": "completed"}
            ...
            def objective_reviewer(token: str, fresh: bool) -> ReviewResult:
                ...
                if target.read_text(encoding="utf-8") != "repaired\n":
                    finding = ReviewFinding(
                        ...
                        {"path": "review_target.txt", "content_sha256": _sha256(target)},
                        "review_target.txt contains the repaired marker",
                    )
                    return ReviewResult(
                        "objective-review-1",
            ...
            def repair(finding: ReviewFinding) -> RepairRecord:
                sdk_turn(
                    "Repair the exact prior findings in this disposable repository: write review_target.txt with exactly "
                    "'repaired' followed by one newline. Do not modify any other file. Return the requested JSON object."
                )
                return RepairRecord(
                    "repair-1",
                    finding.finding_id,
                    DispatchId("multi-authority-review-pilot/multi-authority-review/executor/1"),
                    "repair objective marker through the same SDK thread",
                    "repaired",
                )
            ...
            return {
                ...
                "fixed_rendered_evidence": {
                    ...
                    "artifact_path": "review-rendered-evidence.svg",
                },
                "authorized_changed_paths": _git(root, "status", "--short").splitlines(),
                "final_file_sha256": _sha256(target),
                "lifecycle_kinds": [
                    record.kind
                    for record in ledger.review_lifecycle("multi-authority-review-pilot", "multi-authority-review")
                ],
                "review_count": len(result.review_ids),
                "repair_count": len(result.repair_ids),
                "same_owner_repair": True,
                "architecture_replan": True,
                "accepted_state": ledger.current_state("multi-authority-review-pilot", "multi-authority-review").value,
            }
        except CodexFlowError as exc:
            return {
                "schema": "codex-flow/h4-b-pilot/v1",
