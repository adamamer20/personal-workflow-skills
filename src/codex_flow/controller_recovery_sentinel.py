"""Opt-in real controller recovery sentinel for the canonical execution path."""
...
def run_controller_recovery_sentinel(*, model: str, effort: ReasoningEffort) -> dict[str, Any]:
...
def write_controller_recovery_evidence(path: Path, evidence: dict[str, Any]) -> None:
...
def _run_worker(repository: Path, phase: str, output: Path) -> None:
    completed = subprocess.run(
        (
            sys.executable,
            "-m",
            "codex_flow.controller_recovery_sentinel",
            "--worker",
            phase,
            "--repository",
            str(repository),
...
__all__ = ["run_controller_recovery_sentinel", "write_controller_recovery_evidence"]
